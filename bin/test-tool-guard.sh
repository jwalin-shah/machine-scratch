...
expect_deny "piped head" "Re-run without the pipe"  Bash "system_profiler SPHardwareDataType | head -40"
expect_deny "piped head pmset" "Re-run without the pipe"  Bash "pmset -g | head -30"
...
expect_deny "echo" "jq -n for JSON"            Bash "echo hello"
expect_deny "printf" "jq -n for JSON"          Bash "printf '%s\n' hello"
...
expect_deny "Shell echo" "jq -n for JSON"            Shell "echo hello"
expect_deny "Shell printf" "jq -n for JSON"          Shell "printf hi"
